...
// നിങ്ങൾ നൽകിയ ഫയർബേസ് കോൺഫിഗറേഷൻ
...
const getYouTubeEmbedUrl = (url) => {
  ...
  try {
    ...
    // Added loop and playlist for continuous play
...
export default function App() {
    ...
    switch (currentPage) {
      ...
      case 'productDetail':
        return <ProductDetailPage 
                ...
                />;
      case 'contact':
        return <ContactPage siteSettings={siteSettings} navigateTo={navigateTo} />;
      case 'adminLogin':
  ...
  return (
    // Set theme based on new design system
    <div className="font-sans bg-white min-h-screen text-gray-900">
      ...
      <Footer siteSettings={siteSettings} navigateTo={navigateTo} categories={categories} />
    </div>
  );
}
...
const Header = ({ siteSettings, navigateTo, isAdmin }) => (
  <header className="bg-black text-white p-4 fixed top-0 left-0 right-0 z-50 shadow-lg">
    <div className="container mx-auto flex justify-between items-center">
      ...
      >
        <img 
          src={siteSettings.logoUrl || 'https://placehold.co/100x100/1a202c/amber-500?text=A'} 
          alt="Al Ambar Logo" 
          className="h-12 w-12 rounded-full object-cover border-2 border-amber-500"
        />
        <span className="text-2xl font-bold text-amber-500 tracking-wider">
...
const NavItem = ({ onClick, children }) => (
  <button 
    onClick={onClick}
    className="text-lg font-medium text-white hover:text-amber-500 transition-colors"
  >
    {children}
  </button>
);

const Footer = ({ siteSettings, navigateTo, categories }) => {
  return (
    <footer className="bg-black text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-full bg-gradient-to-r from-amber-400 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/30">
                <span className="text-2xl font-bold text-black">A</span>
              </div>
              <span className="text-2xl font-bold text-white">{siteSettings.siteName || "Al Ambar"}</span>
            </div>
            <p className="text-sm text-gray-400">
              {siteSettings.tagline || "Experience the finest Arabic perfumes, attars, and luxury fragrances."}
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              <li>
                <button onClick={() => navigateTo('home')} className="text-sm text-gray-300 hover:text-amber-500 transition-colors">
                  Home
                </button>
              </li>
              <li>
                <button onClick={() => navigateTo('products')} className="text-sm text-gray-300 hover:text-amber-500 transition-colors">
                  Products
                </button>
              </li>
              <li>
                <button onClick={() => navigateTo('contact')} className="text-sm text-gray-300 hover:text-amber-500 transition-colors">
                  Contact Us
                </button>
              </li>
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {categories.slice(0, 5).map(cat => (
                <li key={cat.id}>
                  <button onClick={() => navigateTo('products', { categoryId: cat.id })} className="text-sm text-gray-300 hover:text-amber-500 transition-colors">
                    {cat.name}
                  </button>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start space-x-2 text-sm text-gray-300">
                <PhoneIcon className="w-4 h-4 mt-0.5 text-amber-500" />
                <span>{siteSettings.phone || "+91 XXXXX XXXXX"}</span>
              </li>
              <li className="flex items-start space-x-2 text-sm text-gray-300">
                <MailIcon className="w-4 h-4 mt-0.5 text-amber-500" />
                <span>{siteSettings.email || "info@alambar.com"}</span>
              </li>
              <li className="flex items-start space-x-2 text-sm text-gray-300">
                <MapPinIcon className="w-4 h-4 mt-0.5 text-amber-500" />
                <span>{siteSettings.address || "Kerala, India"}</span>
              </li>
            </ul>
            <div className="flex space-x-4 mt-4">
              <a href="#" className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-amber-500 hover:text-black transition-colors">
        ...
        <div className="border-t border-gray-800 mt-8 pt-8 text-center text-sm text-gray-500">
          <p>&copy; {new Date().getFullYear()} Al Ambar Perfume Company. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
...
// --- NEW HOMEPAGE (based on user files) ---
const HomePage = ({ siteSettings, categories, featuredProducts, navigateTo, whatsappNumber }) => {
  return (
    <div className="bg-white">
      <HeroSection navigateTo={navigateTo} whatsappNumber={whatsappNumber} />
      <VideoSection videoUrl={siteSettings.videoUrl} />
      <CategorySection categories={categories} navigateTo={navigateTo} />
      <FeaturedProducts products={featuredProducts} navigateTo={navigateTo} whatsappNumber={whatsappNumber} />
    </div>
  );
};

// --- NEW HeroSection ---
const HeroSection = ({ navigateTo, whatsappNumber }) => {
  const heroImage = "https://images.unsplash.com/photo-1541643600914-78b084683601?w=1200&auto=format&fit=crop&q=80";
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent("Hello Al Ambar, I would like to place an order.")}`;
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
        <div className="max-w-2xl">
          <div className="mb-6 inline-block">
            <span className="px-4 py-2 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-full text-sm font-medium backdrop-blur-sm">
              Premium Arabic Fragrances
            </span>
          </div>
          
          <h1 className="text-5xl md:text-7xl font-bold text-white mb-6">
            Discover the <span className="text-amber-500">Essence</span> of Luxury
          </h1>
          
          <p className="text-xl text-white/90 mb-8 leading-relaxed">
            Experience the timeless tradition of Arabic perfumery with Al Ambar's exquisite collection of attars, ouds, and luxury fragrances.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4">
            <button 
              onClick={() => navigateTo('products')}
              className="text-lg h-14 px-8 rounded-md flex items-center justify-center font-semibold bg-gradient-to-r from-amber-400 to-amber-600 text-black shadow-lg shadow-amber-500/30 transition-all duration-300 hover:shadow-xl hover:from-amber-500 hover:to-amber-600"
            >
              Explore Collection
            ...
            <a 
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-lg h-14 px-8 rounded-md flex items-center justify-center font-semibold border-2 border-white text-white hover:bg-white hover:text-black transition-colors backdrop-blur-sm"
            >
              Order on WhatsApp
            </a>
          </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-black mb-4">
            The Art of <span className="text-amber-600">Perfumery</span>
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Discover the craftsmanship and tradition behind Al Ambar's luxury fragrances
          </p>
        </div>
        ...
        <div className="relative aspect-video rounded-2xl overflow-hidden shadow-2xl group bg-black">
          {!isPlaying ? (
            <>
              <img 
                src="https://images.unsplash.com/photo-1557170060-1e8c1d3c0234?w=1200&auto=format&fit=crop&q=80" 
              ...
              <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                <button
                  ...
                  className="w-20 h-20 rounded-full bg-gradient-to-r from-amber-400 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/30 transition-transform duration-300 hover:scale-110"
                  aria-label="Play video"
                >
                  <PlayIcon className="w-8 h-8 text-black ml-1" fill="black" />
                </button>
              </div>
            </>
          ) : isYouTube ? (
        ...
        <div className="mt-8 text-center">
          <p className="text-gray-500 italic">
            "Each fragrance tells a story of tradition, craftsmanship, and timeless elegance"
          </p>
        </div>
      </div>
    </section>
...
// --- NEW CategorySection ---
const CategorySection = ({ categories, navigateTo }) => {
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            From traditional attars to modern luxury fragrances, discover the perfect scent
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((category) => (
            <CategoryCard 
              key={category.id} 
              category={category} 
              onClick={() => navigateTo('products', { categoryId: category.id })}
            />
          ))}
        </div>
      </div>
...
const CategoryCard = ({ category, onClick }) => (
  <div
    onClick={onClick}
    className="group overflow-hidden rounded-lg border border-gray-200 hover:border-amber-500 shadow-md hover:shadow-2xl transition-all duration-300 cursor-pointer"
  >
    <div className="relative aspect-square overflow-hidden">
      <img
        src={category.imageUrl || 'https://placehold.co/400x400/eeeeee/1a202c?text=Category'}
        alt={category.name}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6">
        ...
        {/* <p className="text-white/90 text-sm">{category.description || 'View Collection'}</p> */}
      </div>
    </div>
  </div>
);
...
const FeaturedProducts = ({ products, navigateTo, whatsappNumber }) => {
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-black mb-4">
            Featured <span className="text-amber-600">Fragrances</span>
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
        ...
        <div className="text-center mt-12">
          <button 
            onClick={() => navigateTo('products')}
            className="text-lg h-14 px-8 rounded-md flex items-center justify-center font-semibold border-2 border-amber-600 text-amber-600 hover:bg-amber-600 hover:text-white transition-colors"
          >
            View All Products
          </button>
        </div>
...
// --- NEW ProductCard (replaces old one) ---
const ProductCard = ({ product, onClick, whatsappNumber }) => {
  const message = `Hi Al Ambar, I would like to order: ${product.name} (Price: ₹${product.price}). Product ID: ${product.id}`;
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`;

  return (
    <div
      className="group overflow-hidden rounded-lg border border-gray-200 bg-white shadow-md hover:shadow-2xl transition-all duration-300 flex flex-col"
    >
      <div className="relative aspect-[3/4] overflow-hidden cursor-pointer" onClick={onClick}>
        <img
          src={product.imageUrl || 'https://placehold.co/400x533/eeeeee/1a202c?text=Perfume'}
          alt={product.name}
          className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"
        />
      </div>
      ...
      <div className="p-4 space-y-3 flex-grow flex flex-col">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="w-full mt-auto text-center px-4 py-2 rounded-md font-semibold bg-gradient-to-r from-amber-400 to-amber-600 text-black shadow-lg shadow-amber-500/30 transition-all duration-300 hover:shadow-xl hover:from-amber-500 hover:to-amber-600"
        >
          <ShoppingBagIcon className="w-4 h-4 mr-2 inline-block" />
          Order on WhatsApp
        </a>
      </div>
    </div>
...
// --- Product List Page (Updated to use new card) ---
const ProductListPage = ({ products, categories, selectedCategory, navigateTo, setSelectedCategory, whatsappNumber }) => (
  <div className="container mx-auto px-4 min-h-[60vh]">
    <h1 className="text-4xl font-bold text-center my-12 text-black">Our Products</h1>
    
    {/* Category Filter */}
    <div className="flex justify-center flex-wrap gap-4 mb-12">
      <button 
        onClick={() => setSelectedCategory(null)}
        className={`px-6 py-2 rounded-full font-semibold transition-colors ${!selectedCategory ? 'bg-black text-amber-500' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}
      >
        All
      </button>
      {categories.map(cat => (
        <button
          ...
          className={`px-6 py-2 rounded-full font-semibold transition-colors ${selectedCategory === cat.id ? 'bg-black text-amber-500' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}
        >
          {cat.name}
        </button>
      ))}
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-12 text-center min-h-[60vh]">
        <h1 className="text-3xl text-black">Product not found.</h1>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        <div className="flex flex-col h-full pt-4">
          <h1 className="text-4xl md:text-5xl font-bold text-black">{product.name}</h1>
...
const ContactPage = ({ siteSettings, navigateTo }) => (
  <div className="container mx-auto px-4 py-12 min-h-[60vh]">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      <div className="bg-gray-50 p-8 rounded-lg shadow-lg">
        ...
        <div className="space-y-4">
          <p className="flex items-center text-lg">
            <b className="w-24 text-black">WhatsApp:</b>
      ...
      <div className="rounded-lg shadow-lg overflow-hidden">
        <iframe 
          src={siteSettings.mapEmbedUrl || "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3960.902837392275!2d79.8589025152348!3d6.90222099501237!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ae2596311955555%3A0x546c34cd99f6f488!2sColombo%2C%2BSystem.search!5e0!3m2!1sen!2sus!4v1678888888888!5m2!1sen!2sus"} 
          width="100%" 
          height="450" 
          style={{ border:0 }} 
          allowFullScreen="" 
...
const AdminLoginPage = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-md p-8 bg-white border border-gray-200 rounded-lg shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-black text-amber-500 py-3 rounded-lg font-bold hover:bg-gray-800 transition-colors">
            Login
          </button>
        </form>
      </div>
...
// Admin Dashboard
const AdminDashboard = ({ siteSettings, categories, products, onLogout }) => {
  const [currentTab, setCurrentTab] = useState('dashboard');
  
  const stats = {
    products: products.length || 0,
    categories: categories.length || 0,
    featuredProducts: products.filter(p => p.isFeatured).length || 0,
  };
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="flex border-b mb-8">
        <AdminTab title="Dashboard" id="dashboard" currentTab={currentTab} setCurrentTab={setCurrentTab} />
  ...
  <div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <p className="text-3xl font-bold text-black">{stats.featuredProducts}</p>
      </div>
    </div>
    <p className="text-xl text-gray-600">Select a tab above to manage your site.</p>
  </div>
);
...
const AdminButton = ({ children, onClick, type = "button", isLoading = false, variant = "primary" }) => {
  const colors = {
    primary: "bg-black text-amber-500 hover:bg-gray-800",
